...
class MainActivity : AppCompatActivity(), GameListContract.View {
    ...
    override fun showGameList(gameResults: GameResults) {
        loading = false
        if (gameListAdapter == null) {
            gameListAdapter = GamesListAdapter(gameResults.results)
            rvGamesList.adapter = gameListAdapter
            onClickItem()
            rvGamesList.visibility = View.VISIBLE
            loadingBar.visibility = View.GONE
        } else {
            gameListAdapter?.addItems(gameResults.results)
        }
    }
